...
cloud_ptr_t Scan3D::transformScan(const laser_t& scan) {
    ...
    projector_.projectLaser(scan, cloud, -1);							// transform LaserScan to pcl::PointCloud 
    cloud.header.frame_id = "base_link";
    pcl::fromROSMsg(cloud, cloud_pcl);
    
    searchGround(scan);
    matrix4 tf = createTransformMatrix(1.5);
...
matrix4 Scan3D::createTransformMatrix(double position_factor) {
    double x = pose_.rotation.x;	/* yaw */
    double y = pose_.rotation.y;	/* pitch */
    double z = pose_.rotation.z;	/* roll */
...
void Scan3D::searchGround(const laser_t& scan) {
    if (trigonometric_ground_search_) {
        ...
        std::cout << scan.ranges[0] << " " << pose_.position.z << " " << (pose_.rotation.z * (180.0/acos(-1))) << std::endl << std::endl;
    }
}
...
double Scan3D::searchTrigonometric(const laser_t& scan) {
    ...
    for (int i = 0; i < total_scan; i++) {
        ...
        if (!isnan(tan_alpha)) {
            sum += tan_alpha;
            count += 1;
            distance_sum[0] += distance[0];
            distance_sum[1] += distance[1];
        }
        ceta += scan.angle_increment;
    }

    std::cout << total_scan << " " << count << std::endl;
    
    if (count == 0) return 0.0;
    double distance_average[2] = { distance_sum[0] / static_cast<double>(count),
                                   distance_sum[1] / static_cast<double>(count) }
